    ...
    <div className="min-h-[80vh] flex flex-col items-center justify-center p-4 relative z-10">
      <div className="ui-glass-panel max-w-md w-full p-10 text-center animate-popIn">
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center items-center">
          {showRetry && <button type="button" onClick={onRetry || (() => window.location.reload())} className="ui-button-primary w-full sm:w-auto py-3 px-6">{t('select.retry')}</button>}
          <button type="button" onClick={onBack} className="w-full sm:w-auto px-6 py-3 rounded-xl border-2 border-borderClient text-textSecondary font-bold uppercase tracking-wide hover:border-textMain hover:text-textMain transition-colors">{t('select.back')}</button>
        </div>
      </div>
    </div>
  );
