...
const StatusMessage = React.memo(({ type, text }) => {
    if (!text) return null;
...
export default function PasswordChangePage() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (errorMessage.includes("different from the old password") || errorMessage.includes("same as the old password")) {
                errorMessage = t('profile.errors.password_same_as_old');
            } else {
    ...
    const parsedTips = Array.isArray(securityTips) ? securityTips : (typeof securityTips === 'string' ? JSON.parse(securityTips) : []);
                    ...
                    <form onSubmit={handleSubmit} className="flex flex-col gap-6 md:gap-8">
                        ...
                        <div className="mt-4 bg-body rounded-xl border border-borderClient overflow-hidden transition-colors hover:border-accent">
                            ...
                            >
                                ...
                                <FaChevronDown 
                                    className={`text-textSecondary transition-transform duration-[400ms] ease-[cubic-bezier(0.4,0,0.2,1)] ${isTipsOpen ? 'rotate-180 text-accent' : ''}`} 
                                />
                            </button>

                            <div className={`grid transition-all duration-[400ms] ease-[cubic-bezier(0.4,0,0.2,1)] ${isTipsOpen ? 'grid-rows-[1fr]' : 'grid-rows-[0fr]'}`}>
