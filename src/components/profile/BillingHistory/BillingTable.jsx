...
const BillingTable = ({ history, expandedItems, onToggleRow, dateFormatter }) => {
      ...
      {history.map((item) => {
        ...
        return (
          <div 
            ...
            className={`
              /* ПК Стилі (Таблиця) */
              sm:grid sm:grid-cols-[1.2fr_1.5fr_0.8fr_1.2fr_1fr] sm:px-6 sm:py-5 sm:items-center sm:border-b sm:border-borderClient sm:bg-surface sm:hover:bg-hover sm:cursor-default sm:border-l-0 sm:shadow-none sm:rounded-none sm:last:border-b-0
              
              /* Мобільні Стилі (Картки) */
              flex flex-wrap p-5 rounded-xl border border-borderClient bg-surface shadow-sm relative cursor-pointer overflow-hidden transition-all duration-200 hover:-translate-y-0.5 hover:shadow-md active:-translate-y-0.5 active:shadow-md
              ${rowBorderClass} 
              ${isExpanded ? 'shadow-md -translate-y-0.5' : ''}
            `}
            onClick={() => {
              // Вмикаємо клік по рядку тільки на мобільних
