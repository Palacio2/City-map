        ...
        <div className="bg-surface rounded-2xl p-6 sm:p-12 w-full max-w-[500px] shadow-card border border-borderClient text-center animate-fadeInUp flex flex-col">
          <div className="flex flex-col items-center gap-5">
            ...
            {resendStatus === 'success' && (
              <p className="text-success font-semibold m-0">{t('auth.forgot_pass.success_title')}</p>
            )}
            
            {resendStatus === 'error' && (
              <p className="text-danger font-semibold m-0">{t('auth.errors.generic')}</p>
            )}
                ...
                >
                  {isResending ? (
                    <>
                      ...
                      {t('auth.forgot_pass.sending')}
                    </>
                  ) : (
                    <>
                      <FaRedo />
                      {t('auth.success_reg.resend_btn')}
                    </>
                  )}
                </button>
              )}
