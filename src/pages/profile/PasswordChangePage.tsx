          ...
          <form onSubmit={onSubmit} className="flex flex-col gap-6 md:gap-8">
            ...
            <div className="text-center border-b border-borderClient pb-6 mb-2">
              <h2 className="font-heading text-textMain text-2xl font-bold mb-2">
                {t('profile.password_page.form_title')}
              </h2>
              <p className="text-textSecondary text-[0.9rem] m-0">
                {t('profile.password_page.form_subtitle')}
              </p>
            </div>
            ...
            {statusMessage.text && (
              <div 
                ...
                role="alert"
              >
                {statusMessage.type === 'success' ? <FaCheckCircle className="shrink-0 text-lg" /> : <FaTimesCircle className="shrink-0 text-lg" />}
            ...
            <div className="flex flex-col gap-5">
              <div className="flex flex-col gap-2">
                <label htmlFor="newPassword" className="flex items-center gap-2 font-semibold text-textMain text-[0.9rem] font-heading uppercase tracking-wide">
                ...
                <div className="relative flex items-center w-full">
                  <input
                    ...
                    className={`w-full p-4 pr-12 bg-body border rounded-lg text-textMain font-body text-base transition-all focus:outline-none focus:ring-2 focus:ring-accent/20 disabled:opacity-60 disabled:cursor-not-allowed ${form.formState.errors.newPassword ? 'border-danger focus:border-danger' : 'border-borderClient focus:border-accent'}`}
                  ...
                  <button 
                    type="button" 
                    className="absolute right-3 top-1/2 -translate-y-1/2 bg-transparent border-none text-textSecondary cursor-pointer p-2 flex items-center justify-center transition-all hover:text-textMain hover:scale-110" 
                ...
                {form.formState.errors.newPassword && (
                  <span className="text-danger text-sm mt-1">{form.formState.errors.newPassword.message}</span>
                )}
              </div>

              <div className="flex flex-col gap-2">
                <label htmlFor="confirmPassword" className="flex items-center gap-2 font-semibold text-textMain text-[0.9rem] font-heading uppercase tracking-wide">
                  <FaLock className="text-accent text-[0.9rem]" />
                ...
                <div className="relative flex items-center w-full">
                  <input
                    ...
                    className={`w-full p-4 pr-12 bg-body border rounded-lg text-textMain font-body text-base transition-all focus:outline-none focus:ring-2 focus:ring-accent/20 disabled:opacity-60 disabled:cursor-not-allowed ${form.formState.errors.confirmPassword ? 'border-danger focus:border-danger' : 'border-borderClient focus:border-accent'}`}
                ...
                {form.formState.errors.confirmPassword && (
                  <span className="text-danger text-sm mt-1">{form.formState.errors.confirmPassword.message}</span>
                )}
              </div>
            </div>
            ...
            <div className="mt-4 bg-body rounded-xl border border-borderClient overflow-hidden transition-colors hover:border-accent">
              <button 
                type="button" 
                className={`flex items-center justify-between p-5 cursor-pointer w-full bg-transparent border-b border-transparent text-textMain transition-all hover:bg-accent/5 ${isTipsOpen ? 'border-borderClient bg-accent/5' : ''}`} 
