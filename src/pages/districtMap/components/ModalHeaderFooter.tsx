...
import { useFavorites } from '@pages/favorites/context/FavoritesContext';
import { Button } from '@ui/Button';
import { useFormat } from '@hooks/useFormat';
import { CloseButton, FavoriteButton, CommentButton } from './ModalButtons';
import type { TransformedDistrict, TransformedFilterData, GeneralStats } from '@utils/dataTransformers';
...
export const HeaderSection = ({
  ...
  const isFav = useMemo(() => {
    return district?.id ? isFavorite(String(district.id)) : false;
  }, [district, isFavorite]);
  ...
  const currencyInfo = useMemo(() => getCurrencyInfo('EU'), [getCurrencyInfo]);

  const quickStatsConfig: Array<{ key: keyof GeneralStats; label: string; formatter: (val: unknown) => unknown }> = useMemo(() => [
    { key: 'population', label: 'common.fields.population', formatter: (val) => formatNumber(val as number) },
    { key: 'averageSalary', label: 'average_salary', formatter: (val) => formatPrice(val as number, currencyInfo) },
    { key: 'unemploymentRate', label: 'unemployment_rate', formatter: (val) => `${val}%` },
    { key: 'propertyPrice', label: 'common.fields.propertyPricePerSqm', formatter: (val) => formatPrice(val as number, currencyInfo) },
    { key: 'average_rent_price', label: 'common.fields.average_rent_price', formatter: (val) => formatPrice(val as number, currencyInfo) }
  ...
  const handleHeartClick = async () => {
    if (district.id) {
      await toggleFavorite(district as Partial<TransformedDistrict> & { id: string | number });
    }
  };

  return (
    <>
      <div className="relative h-[260px] md:h-[420px] bg-black shrink-0 overflow-hidden flex flex-col rounded-t-3xl md:rounded-t-[var(--radius-lg)]">
         {photo_url && (
        ...
        <div className="relative z-10 flex-1 p-4 pt-4 md:p-8 md:pt-[max(2rem,env(safe-area-inset-top))] flex flex-col text-white justify-between">
          <div className="flex justify-between items-start w-full">
            <div className="hidden md:inline-flex items-center gap-2 bg-black/50 backdrop-blur-md px-3.5 py-1.5 rounded-full font-body text-[0.75rem] font-medium text-white/90 border border-white/20 shadow-sm">
              <span className="w-1.5 h-1.5 bg-success rounded-full shadow-[0_0_8px_var(--success-color)]" />
          ...
          <div className="flex flex-col gap-2 md:gap-4 mt-auto">
            <div>
              <h1 className="font-heading text-[1.75rem] md:text-[3rem] font-semibold m-0 leading-tight tracking-tight text-white drop-shadow-[0_4px_20px_rgba(0,0,0,0.5)]">
                {name}
              </h1>
              {formattedDate && (
                    ...
                    <div key={key} className="min-w-[130px] md:min-w-[180px] flex-none bg-white/10 backdrop-blur-md border border-white/15 rounded-md p-2.5 md:py-3 md:px-4 flex items-center gap-2.5 md:gap-3 transition-all hover:bg-white/20">
                      ...
                      <div className="flex flex-col overflow-hidden">
                        ...
                        <span className="font-body text-[0.85rem] md:text-[0.95rem] font-semibold text-white whitespace-nowrap">
                            {formatter ? String(formatter(value)) : String(value)}
                        </span>
                      </div>
                    </div>
                  );
...
export const ModalFooter = ({ onClose }: { readonly onClose: () => void }) => {
