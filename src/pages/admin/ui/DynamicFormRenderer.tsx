...
const CompactFieldRenderer = React.memo(({ field, value, onChange, readOnly }: { field: any; value: any; onChange: (key: string, val: any) => void; readOnly?: boolean }) => {
...
export const DynamicFormRenderer: React.FC<DynamicFormRendererProps> = React.memo(({ fieldsConfig, formData, onChange, readOnly = false }) => {
    ...
    const groupedFields = useMemo(() => {
        return (fieldsConfig || []).reduce((acc: Record<string, any[]>, field: any) => {
    ...
    return (
        <div className="flex flex-col gap-6">
            {Object.entries(groupedFields).map(([group, fields]) => {
                const textFields = fields.filter((f: any) => f.ui_component === 'textarea' || f.type === 'text');
                const compactFields = fields.filter((f: any) => f.ui_component !== 'textarea' && f.type !== 'text');
                    ...
                    <div key={group} className="flex flex-col">
                        ...
                        {compactFields.length > 0 && (
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
                                {compactFields.sort((a: any, b: any) => (a.sort_order || 0) - (b.sort_order || 0)).map((field: any) => (
                        ...
                        {textFields.length > 0 && (
                            <div className={`grid grid-cols-1 gap-3 ${compactFields.length > 0 ? 'mt-3' : ''}`}>
                                {textFields.sort((a: any, b: any) => (a.sort_order || 0) - (b.sort_order || 0)).map((field: any) => (
                                    <CompactFieldRenderer
                                        key={field.id || field.key}
                                        field={field}
                                        value={formData[field.key]}
