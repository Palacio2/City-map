...
// 1. Призначення кольорів районам (щоб сусідні не зливалися)
...
// 2. Автоматичне фокусування камери на районах
    ...
    useEffect(() => {
        ...
        if (features.length > 0) {
            try {
                ...
                console.error("MapFitBounds error:", e);
            } 
        }
...
// 3. Всеїдний нормалізатор міток (парсить будь-який формат і перевертає координати)
export const normalizePoiData = (rawPoi) => {
    ...
    return flatPois.map(p => {
        ...
        // Автовиправлення переплутаних координат (Довгота <-> Широта)
        if ((lat > 14 && lat < 40 && lng > 44 && lng < 55) || (lat > 10 && lat < 45 && lng > 40 && lng < 65)) {
...
// 4. Створення красивих маркерів на карті
export const createEmojiIcon = (key, source, fieldsConfig, size = 28) => {
    const field = fieldsConfig?.find(f => f.key === key || f.key === `${key}_count`);
    const emoji = field?.icon || '📍';
